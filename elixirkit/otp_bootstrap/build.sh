...
if [ -d $openssl_src_dir ]; then
  echo "`realpath --relative-to $PWD $openssl_src_dir` already exists"
else
...
if [ -d $openssl_rel_dir ]; then
  echo "`realpath --relative-to $PWD $openssl_rel_dir` already exists"
else
  (
    cd $openssl_src_dir
...
if [ -d $otp_src_dir ]; then
  echo "`realpath --relative-to $PWD $otp_src_dir` already exists"
else
...
if [ -d $otp_rel_dir ]; then
  echo "`realpath --relative-to $PWD $otp_rel_dir` already exists"
else
  (
    cd $otp_src_dir
    git clean -dfx
